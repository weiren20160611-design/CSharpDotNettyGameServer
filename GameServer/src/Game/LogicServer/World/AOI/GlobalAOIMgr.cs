...
namespace Game.LogicServer
{
    public class GlobalAOIMgr : AOIMgr
    {
        ...
        {
            NavSystemUpdate(dt);
            SkillAndBuffSystemUpdate(dt);
        }
        void NavSystemUpdate(float dt)
        {
        ...
        void SkillAndBuffSystemUpdate(float dt)
        {
            foreach (var player in this.players.Values)
            {
        ...
        {
            this.NavSystemLateUpdate(dt);
        }

        
        void NavSystemLateUpdate(float dt)
        {
            foreach (var player in this.players.Values)
            {
                if (player.uStatus.status == (int)CharactorStatus.Run)
                {
                    this.mapWrapper.NavLateUpdate(this, player, dt);
                }
            }
        }
        ...
        public override void BroadMessageToAOI(BaseEntity player, Message msg, long ignorePlayerId = -1, List<GM_PlayerEntity> toPlayers = null)
        {
