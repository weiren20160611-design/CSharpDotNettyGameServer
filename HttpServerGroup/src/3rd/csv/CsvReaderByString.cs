...
public class CsvReaderByString{
	...
	public CsvReaderByString(string parseString)
	{
		...
		{
			throw new System.Exception("CSV文件的格式有错误");
		}
	}
	...
	private void CheckRowValid(int row)
	{
		...
		if (row > RowCount)
		{
			throw new System.Exception("没有当前行的数据");   
		}  
	}
	...
	private void CheckMaxRowValid(int maxRow)
	{
		...
		{
			throw new System.Exception("行数不能等于0或小于-1");    
		} 
		if (maxRow > RowCount)
		{
			throw new System.Exception("没有当前行的数据");   
	...
	private void CheckColValid(int col)
	{
		if (col <= 0)
		{
			throw new System.Exception("列数不能小于0");    
		} 
		if (col > ColCount)
		{
			throw new System.Exception("没有当前列的数据");   
		}
	}
	...
	private void CheckMaxColValid(int maxCol)
	{
		...
		{
			throw new System.Exception("列数不能等于0或小于-1");    
		} 
		if (maxCol > ColCount)
		{
			throw new System.Exception("没有当前列的数据");   
	...
	private void AddNewDataLine(string newDataLine)
	{
		...
		if (oddStartQuota)
		{
			throw new System.Exception("数据格式有问题");
		}
	...
	private string GetHandleData(string fileCellData)
	{
		...
		if (IfOddStartQuota(fileCellData))
		{
			...
			{
				throw new System.Exception("数据引号无法匹配" + fileCellData);
			}    
		}
		else
		{
